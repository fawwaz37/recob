...
export class AuthService {
  ...
  async signUp(dto: CreateUserDto) {
    ...
    dto.password = hash;
    const users = await this.userService.createUser(dto);
  ...
  async forgotPassword(email: string) {
    try {
      const otp = await this.userService.createOTP(email);
  ...
  async verificationCode(body: { email: string; code: string }) {
    const data = await this.userService.findOtp(body.email, body.code);
    if (!data) {
  ...
  async resetPassword(dto: ResetPassDto) {
    const code = await this.userService.findOtp(dto.email, dto.code);
    if (!code) {
      throw new NotFoundException(
        'Code OTP dan Email Tidak Sesuai / Code OTP Expired',
      );
    }
    const hash = await argon2.hash(dto.password);
    const data = await this.userService.updatePassword(dto.email, hash);
    if (!data) {
      throw new NotFoundException('Data User Tidak Ditemukan');
    }
    await this.userService.deleteOtp(code.code);
